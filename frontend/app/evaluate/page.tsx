...
/*
 * ─────────────────────────────────────────────────────────────────────────────
 * evaluate/page.tsx  —  Criterion-level Evaluation View (3-panel layout)
 *
 * CURRENT STATE: All data on this page is MOCK / HARDCODED.
 * Nothing here reads from the database or calls any real API.
 *
 * TEAMMATE TODO (when Layers 3–5 are ready):
 *   Replace the three mock objects below — BIDDERS, CRITERIA, VERDICTS —
 *   with real API calls to your evaluation engine endpoints.
 *   Search for "TEAMMATE TODO" in this file to find every exact spot.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ── TEAMMATE TODO (1/3) ───────────────────────────────────────────────────────
// Replace this hardcoded array with a real API call.
// Expected endpoint (once Layer 3 is live):
//   GET /api/v1/tenders/{tenderId}/bidders
// Expected response shape per item:
//   { id: string, name: string, overall: "PASS" | "FAIL" | "REVIEW", score: number }
//
// Suggested implementation:
//   const [bidders, setBidders] = useState([]);
//   useEffect(() => {
//     axios.get(`/api/v1/tenders/${tenderId}/bidders`).then(r => setBidders(r.data));
//   }, [tenderId]);
//
// Until then, BIDDERS below is used as a stand-in.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── TEAMMATE TODO (2/3) ───────────────────────────────────────────────────────
// Replace this hardcoded array with a real API call.
// Expected endpoint (once Layer 3a — Tender Criterion Extractor — is live):
//   GET /api/v1/tenders/{tenderId}/criteria
// Expected response shape per item:
//   { id: string, label: string, mandatory: boolean }
//
// The criterion list must be officer-approved before bids are evaluated
// (see Section 6 of the proposal — no criterion locked in by machine alone).
// ─────────────────────────────────────────────────────────────────────────────
...
// ── TEAMMATE TODO (3/3) ───────────────────────────────────────────────────────
// Replace this hardcoded object with a real API call.
// Expected endpoint (once Layer 5 — Evaluation Engine — is live):
//   GET /api/v1/tenders/{tenderId}/bidders/{bidderId}/verdicts
// Expected response shape:
//   Record<criterionId, { verdict: "PASS"|"FAIL"|"REVIEW", conf: number, evidence: string }>
//
// Call this whenever selectedBidder changes (put it in a useEffect).
// ─────────────────────────────────────────────────────────────────────────────

const VERDICTS: Record<
  string,
  Record<string, { verdict: Verdict; conf: number; evidence: string }>
> = {
  "B-01": {
...
for (const b of BIDDERS.slice(1)) {
  ...
  for (const c of CRITERIA) {
    const v: Verdict =
      b.overall === "FAIL" && ["C-02", "C-05", "C-06"].includes(c.id)
        ? "FAIL"
        : b.overall === "REVIEW" && c.id === "C-05"
          ? "REVIEW"
          : "PASS";
    VERDICTS[b.id][c.id] = {
      verdict: v,
      conf: Math.floor(Math.random() * 15) + 84,
      evidence: "—",
    };
  }
}

// ── TEAMMATE TODO — EVIDENCE PANEL (critical) ─────────────────────────────────
// This is why the right panel does NOT update when you click a different criterion.
//
// ROOT CAUSE:
//   EVIDENCE below is a single static object. The right panel always renders
//   from it regardless of which criterion or bidder is selected.
//
// WHAT NEEDS TO HAPPEN:
//   1. Replace EVIDENCE with a per-criterion evidence lookup, keyed by
//      [bidderId][criterionId]. Shape should match the Evidence Object
//      from Layer 3b (see proposal Section 7.3):
//        {
...
//          threshold: string,
//          confidence: number,
//          udin: "PASSED" | "FAILED" | "N/A",
//          itr:  "MATCH"  | "MISMATCH" | "N/A",
//        }
//
//   2. Expected endpoint (once Layer 4 — Evidence Graph — is live):
//        GET /api/v1/tenders/{tenderId}/bidders/{bidderId}/evidence/{criterionId}
//
//   3. Fetch this inside a useEffect that depends on BOTH
//      selectedBidder.id and selectedCriterion.id — so the panel
//      re-renders every time either changes.
//
//   Example:
//     const [evidence, setEvidence] = useState(null);
//     useEffect(() => {
//       axios.get(
//         `/api/v1/tenders/${tenderId}/bidders/${selectedBidder.id}/evidence/${selectedCriterion.id}`
//       ).then(r => setEvidence(r.data));
//     }, [selectedBidder.id, selectedCriterion.id]);
//
//   4. In the JSX below, replace every EVIDENCE.xyz reference with evidence?.xyz
//      and add a loading/empty state for when evidence is null.
//
// Until the API exists, this panel will always show the same ABC Constructions
// C-05 balance sheet data no matter what you click. That is expected for now.
// ─────────────────────────────────────────────────────────────────────────────
...
function ConfBar({ pct }: { pct: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span
        style={{ fontSize: 11, color, fontFamily: "'IBM Plex Mono',monospace" }}
      >
        {pct}%
      </span>
    </div>
    ...
    >
      ...
      >
        <div>
          ...
          >
            Tender #CRPF/2026/CON/034 · Construction Services · Bhopal HQ
            Complex
          </span>
        </div>
        <button
        ...
        <div style={{ flex: 1, overflowY: "auto", background: "var(--navy)" }}>
          ...
          >
            <div>
              ...
              >
                PAN: AABCA1234N · GST: 22AAACA1234N1Z5 · CIN:
                U45200MH2009PTC194587
              </div>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "var(--navy-mid)" }}>
                {["ID", "CRITERION", "VERDICT", "CONFIDENCE", "EVIDENCE"].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
        ...
        >
          ...
          <div style={{ padding: "16px" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                Page {EVIDENCE.page} · BBox {EVIDENCE.bbox} · Hash{" "}
                {EVIDENCE.hash}
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  <span>III Total Income</span>
                  <span>7,43,45,288</span>
                </div>
              </div>
              <div
                style={{
              ...
              >
                ✓ Attested by CA · UDIN: 24000XXXXX5B4V3K
                <br />✓ Verified at ICAI on 18-Apr-2026 · Signatory PAN on file
              </div>
            </div>
            ...
            >
              {[
                ...
                {
                  label: `CONFIDENCE ${EVIDENCE.confidence}%`,
                  ok: EVIDENCE.confidence >= 85,
                },
              ].map((b) => (
                <span
                  key={b.label}
